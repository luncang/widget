...
public class ScrollerNumberPicker extends View {
     ...
     * 默认字体颜色
     */
    private int normalColor = 0xE61A5F;
    /**
     * 选中时候的字体颜色
     */
    ...
    private void defaultMove(int move) {
        //这样写会报java.util.ConcurrentModificationException异常
//        for (ItemObject item : itemList) {
//            item.newY(move);
//        }
        if (itemList != null) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        selectedFont = attribute.getDimension(
                R.styleable.NumberPicker_selecredTextSize, 16.0f);
        itemNumber = attribute.getInt(R.styleable.NumberPicker_itemNumber, 7);
        normalColor = attribute.getColor(
                R.styleable.NumberPicker_normalTextColor, 0xE61A5F);
        selectedColor = attribute.getColor(
                R.styleable.NumberPicker_selecredTextColor, 0x0000ffff);
